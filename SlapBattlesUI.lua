...
-- Auto Collect Slapple Feature
local autoCollectSlapple = false
local lastPosition = nil

local function collectSlapples()
    while autoCollectSlapple do
        local slapplesFolder = game.Workspace:FindFirstChild("Arena") and game.Workspace.Arena:FindFirstChild("island5") and game.Workspace.Arena.island5:FindFirstChild("Slapples")

        if slapplesFolder then
            for _, slapple in pairs(slapplesFolder:GetChildren()) do
                if slapple:IsA("Part") and (slapple.Name == "Slapple" or slapple.Name == "GoldenSlapple") then
                    if player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
                        -- Save the current position
                        lastPosition = player.Character.HumanoidRootPart.CFrame

                        -- Teleport to Slapple
                        player.Character.HumanoidRootPart.CFrame = slapple.CFrame
                        wait(0.5) -- Ensure collection
...
-- Add Auto Collect Slapple Toggle Button
FeaturesTab:addToggle("Auto Collect Slapple", function(state)
    autoCollectSlapple = state
    if state then
        collectSlapples()
    end
end, false)
